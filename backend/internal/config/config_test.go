...
func resetViperWithJWTSecret(t *testing.T) {
	...
	dataDir := t.TempDir()
	require.NoError(t, os.WriteFile(filepath.Join(dataDir, "config.yaml"), []byte("{}\n"), 0o600))
...
func TestConfigAddressHelpers(t *testing.T) {
	...
	if strings.Contains(dbCfg.DSN(), ":@") {
		t.Fatalf("DatabaseConfig.DSN() should not include password separator when empty")
	}

	dbCfg.Password = "secret"
	if !strings.Contains(dbCfg.DSN(), "goldendb:secret@tcp(localhost:3306)") {
		t.Fatalf("DatabaseConfig.DSN() missing password")
	}

	dbCfg.Password = ""
	if strings.Contains(dbCfg.DSNWithTimezone("UTC"), ":@") {
	...
	if !strings.Contains(dbCfg.DSNWithTimezone("UTC"), "loc=UTC") {
...
func TestDatabaseDSNWithTimezone_WithPassword(t *testing.T) {
	...
	if !strings.Contains(got, "u:p@tcp(localhost:3306)") {
		t.Fatalf("DSNWithTimezone should include password: %q", got)
	}
	if !strings.Contains(got, "loc=UTC") {
		t.Fatalf("DSNWithTimezone should include loc=UTC: %q", got)
	}
}
