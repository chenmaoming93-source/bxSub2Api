...
> **版本：1.2.3**
> **日期：2026-09-02**
> **变更摘要：** 新增分组级请求安全检查、规则判定、按分组配置超时、采集、采样、管理页面及多实例配置缓存同步；补充独立安全日志页面、检查状态可视化、风险维度可读化展示，以及严格按配置时间执行的日志生命周期清理。
...
- 连接超时；
- 读取超时；
- 总超时由每次请求的分组 `TimeoutMS` context 控制，共享客户端不设置更短的固定超时；
- 限制响应体大小；
- 不进行无限重试；
...
### v1.2.3 按分组配置执行 SingGuard 超时

验证发现：分组安全检查配置为 10000ms 时，SingGuard 客户端默认的固定 5 秒 HTTP 总超时会更早终止请求。现移除共享客户端的固定总超时，由每次请求携带的分组 `TimeoutMS` context 统一控制，避免不同分组之间相互影响。
