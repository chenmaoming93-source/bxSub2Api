...
func NewPlatformAPIKeyService(
	...
	return &PlatformAPIKeyService{keys: keys, keyGen: keyGen}
}

// GetOrCreatePlatformKey returns the platform-scoped key for (userID, platform, groupID).
...
func (s *PlatformAPIKeyService) GetOrCreatePlatformKey(ctx context.Context, userID int64, platform string, groupID int64) (*APIKey, error) {
	...
	existing, err := platformRepo.GetByUserIDPlatformAndGroup(ctx, userID, platform, groupID)
	if err == nil {
		return existing, nil
	}
	...
	if err := s.keys.Create(ctx, created); err != nil {
		if errors.Is(err, ErrAPIKeyExists) {
			existing, readErr := platformRepo.GetByUserIDPlatformAndGroup(ctx, userID, platform, groupID)
			if readErr == nil {
				return existing, nil
			}
