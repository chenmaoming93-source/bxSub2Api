...
type CreateBackupJobRequest struct {
	BackupType     string `json:"backup_type" binding:"required,oneof=goldendb redis full"`
	UploadToS3     bool   `json:"upload_to_s3"`
	S3ProfileID    string `json:"s3_profile_id"`
	GoldenDBID     string `json:"goldendb_profile_id"`
...
func (h *DataManagementHandler) ListSourceProfiles(c *gin.Context) {
	...
	if sourceType != "goldendb" && sourceType != "redis" {
		response.BadRequest(c, "source_type must be goldendb or redis")
		return
	}
