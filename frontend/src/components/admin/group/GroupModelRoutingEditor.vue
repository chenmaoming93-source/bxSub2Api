<template>
          ...
          <div class="min-w-0 flex-1 space-y-3">
            ...
            <div v-for="candidate in rule.candidates" :key="candidateKey(candidate)" class="relative w-full min-w-0 space-y-2 rounded-md bg-gray-50 p-3 dark:bg-dark-700/50">
              ...
              <div class="grid min-w-0 gap-2 md:grid-cols-3">
                <div><label class="input-label text-xs">{{ t('admin.groups.modelRouting.priority', 'Priority') }}</label><input v-model.number="candidate.priority" @change="sortCandidates(rule)" type="number" min="0" step="1" class="input text-sm" /></div>
                <div>
  ...
  <p v-if="candidate.maxConcurrency == null" class="mt-1 text-[10px] text-gray-500">无限制，不参与账号额度分配</p>
                  ...
                  <p v-if="props.groupId" class="mt-1 text-[10px] text-gray-500">
                    {{ t('admin.groups.modelRouting.effectiveMaxConcurrency') }}：
                    <span v-if="candidate.effectiveMaxConcurrency === undefined">{{ t('admin.groups.modelRouting.effectiveMaxConcurrencyUnavailable') }}</span>
                    <span v-else-if="candidate.effectiveMaxConcurrency === null">{{ t('admin.groups.modelRouting.scheduleUnlimited') }}</span>
                    <span v-else>{{ candidate.effectiveMaxConcurrency }}</span>
                  </p>
                </div>
              </div>
              <ModelRouteConcurrencyScheduleEditor
                v-if="candidate.accounts.length === 1"
                :group-id="props.groupId"
                :route-alias="rule.alias"
                :account-id="candidate.accounts[0].id"
                v-model="candidate.schedules"
              />
...
import { addRoutingCandidate, createEmptyRoutingCandidate, extractUpstreamModel, sortRoutingCandidates, type RoutingEditorAccount, type RoutingEditorCandidate, type RoutingEditorRule } from './groupModelRoutingEditor'
import { toSchedulePayload, validateScheduleDrafts } from './modelRouteConcurrencySchedule'
import type { RoutingEditorScheduleUpdate } from './groupModelRoutingEditor'
...
const select = (candidate: RoutingEditorCandidate, account: RoutingEditorAccount) => {
  ...
  if (!candidate.accounts.some(item => item.id === account.id)) {
    ...
    candidate.effectiveMaxConcurrency = undefined
...
const sortCandidates = (rule: RoutingEditorRule) => sortRoutingCandidates(rule)
...
const getConcurrencyScheduleUpdates = (): RoutingEditorScheduleUpdate[] => {
  const updates: RoutingEditorScheduleUpdate[] = []
  for (const rule of rules.value) for (const candidate of rule.candidates) {
    if (!candidate.accounts.length || !candidate.schedules?.length) continue
    const validation = validateScheduleDrafts(candidate.schedules)
    if (validation) throw new Error(validation)
    updates.push({
      route_alias: rule.alias.trim(),
      account_id: candidate.accounts[0].id,
      schedules: toSchedulePayload(candidate.schedules),
    })
  }
  return updates
}
watch(() => props.groupId, async (groupId) => {
  if (!groupId) return
  for (const rule of rules.value) {
    for (const candidate of rule.candidates) candidate.effectiveMaxConcurrency = undefined
  }
  try {
    const [refs, concurrency] = await Promise.all([
      adminAPI.groups.listModelRouteReferences(groupId),
      adminAPI.groups.listModelRouteConcurrency(groupId).catch(() => []),
    ])
    for (const rule of rules.value) {
      for (const candidate of rule.candidates) {
        ...
        const concurrencyMatch = concurrency.find(item => item.route_alias === rule.alias && item.account_id === account.id)
        if (match) {
        ...
        if (concurrencyMatch) candidate.effectiveMaxConcurrency = concurrencyMatch.effective_max_concurrency
      }
    }
...
defineExpose({ isValid, getConcurrencyUpdates, getConcurrencyScheduleUpdates })
