...
//
// 支持多种检测方式：
//  1. GoldenDB/MySQL duplicate-entry error text or code
//  2. Generic duplicate-key/unique-constraint text from wrapped drivers
//
...
func isUniqueConstraintViolation(err error) bool {
	...
	// as the upstream MySQL driver, so keep this check driver-agnostic.
	msg := strings.ToLower(err.Error())
	return strings.Contains(msg, "duplicate key") ||
		strings.Contains(msg, "unique constraint") ||
		strings.Contains(msg, "duplicate entry") ||
		strings.Contains(msg, "error 1062") ||
		strings.Contains(msg, "errno 1062")
}
