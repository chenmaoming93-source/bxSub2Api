...
func (d *DatabaseConfig) DSN() string {
	return d.goldenDBDSN(d.DBName, "UTC")
}

// DSNWithTimezone returns a GoldenDB/MySQL-compatible DSN with timezone setting.
func (d *DatabaseConfig) DSNWithTimezone(tz string) string {
	if tz == "" {
		tz = "Asia/Shanghai"
	}
	return d.goldenDBDSN(d.DBName, tz)
}

func (d *DatabaseConfig) goldenDBDSN(dbName, tz string) string {
	params := url.Values{}
	params.Set("charset", "utf8mb4")
	params.Set("parseTime", "true")
	params.Set("loc", tz)
	if tlsMode := goldenDBTLSMode(d.SSLMode); tlsMode != "" {
		params.Set("tls", tlsMode)
	}

	auth := url.QueryEscape(d.User)
	if d.Password != "" {
		auth += ":" + url.QueryEscape(d.Password)
	}
	return fmt.Sprintf(
		"%s@tcp(%s)/%s?%s",
		auth,
		net.JoinHostPort(d.Host, fmt.Sprintf("%d", d.Port)),
		dbName,
		params.Encode(),
	)
}

func goldenDBTLSMode(raw string) string {
	switch strings.ToLower(strings.TrimSpace(raw)) {
...
func setDefaults() {
	...
	viper.SetDefault("database.user", "goldendb")
