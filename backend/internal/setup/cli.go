...
func RunCLI() error {
	...
	for {
		cfg.Database.Host = promptString(reader, "GoldenDB Host", "localhost")
	...
	for {
		cfg.Database.Port = promptInt(reader, "GoldenDB Port", 3306)
	...
	for {
		cfg.Database.User = promptString(reader, "GoldenDB User", "goldendb")
	...
	cfg.Database.Password = promptPassword("GoldenDB Password")
