  ...
  <BaseDialog :show="show" :title="t('admin.users.defaultQuota.title', 'Default model daily token quotas for new users')" width="wide" @close="$emit('close')">
    <div class="space-y-4">
      ...
      <div v-else class="space-y-3">
        <div v-for="(row, index) in rows" :key="row.key" class="grid items-end gap-2 rounded-lg border border-gray-200 p-3 dark:border-dark-700 md:grid-cols-[1fr_220px_auto]">
          ...
          <label class="block text-xs text-gray-500">
            ...
            <input v-model.number="row.daily_limit_tokens" data-test="default-quota-limit" type="number" min="0" step="1" class="input mt-1 min-w-[200px] text-sm" :placeholder="t('admin.users.defaultQuota.unlimited')" />
          </label>
