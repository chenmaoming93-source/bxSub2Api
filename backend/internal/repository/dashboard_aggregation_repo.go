...
func (r *dashboardAggregationRepository) CleanupUsageBillingDedup(ctx context.Context, cutoff time.Time) error {
	for {
		if _, err := r.sql.ExecContext(ctx, `
			INSERT IGNORE INTO usage_billing_dedup_archive (request_id, api_key_id, request_fingerprint, created_at)
		...
		res, err := r.sql.ExecContext(ctx, `
			...
			WHERE id IN (
				SELECT id FROM (
					SELECT id FROM usage_billing_dedup
					WHERE created_at < ?
					ORDER BY id
					LIMIT ?
				) AS victims
...
func (r *dashboardAggregationRepository) insertHourlyActiveUsers(ctx context.Context, start, end time.Time) error {
	...
	query := `
		INSERT IGNORE INTO usage_dashboard_hourly_users (bucket_start, user_id)
		SELECT DISTINCT
			CONVERT_TZ(DATE_FORMAT(CONVERT_TZ(created_at, '+00:00', ?), '%Y-%m-%d %H:00:00'), ?, '+00:00') AS bucket_start,
			user_id
		FROM usage_logs
...
func (r *dashboardAggregationRepository) insertDailyActiveUsers(ctx context.Context, start, end time.Time) error {
	...
	query := `
		...
		SELECT DISTINCT
			DATE(CONVERT_TZ(bucket_start, '+00:00', ?)) AS bucket_date,
			user_id
...
func (r *dashboardAggregationRepository) upsertHourlyAggregates(ctx context.Context, start, end time.Time) error {
	...
	query := `
		...
		SELECT hourly.bucket_start, hourly.total_requests, hourly.input_tokens,
			hourly.output_tokens, hourly.cache_creation_tokens, hourly.cache_read_tokens,
			hourly.total_cost, hourly.actual_cost, hourly.account_cost,
			hourly.total_duration_ms, COALESCE(user_counts.active_users, 0), NOW()
		FROM (
			SELECT
				CONVERT_TZ(DATE_FORMAT(CONVERT_TZ(created_at, '+00:00', ?), '%Y-%m-%d %H:00:00'), ?, '+00:00') AS bucket_start,
		...
		LEFT JOIN (
			...
			WHERE bucket_start >= ? AND bucket_start < ?
			GROUP BY bucket_start
		) AS user_counts ON user_counts.bucket_start = hourly.bucket_start
		ON DUPLICATE KEY UPDATE
			...
			actual_cost = VALUES(actual_cost), account_cost = VALUES(account_cost),
			total_duration_ms = VALUES(total_duration_ms), active_users = VALUES(active_users),
			computed_at = VALUES(computed_at)
	`
	_, err := r.sql.ExecContext(ctx, query, tzName, tzName, start, end, start, end)
	return err
}
...
func (r *dashboardAggregationRepository) upsertDailyAggregates(ctx context.Context, start, end time.Time) error {
	...
	query := `
		...
		SELECT daily.bucket_date, daily.total_requests, daily.input_tokens,
			daily.output_tokens, daily.cache_creation_tokens, daily.cache_read_tokens,
			daily.total_cost, daily.actual_cost, daily.account_cost,
			daily.total_duration_ms, COALESCE(user_counts.active_users, 0), NOW()
		FROM (
			SELECT
				DATE(CONVERT_TZ(bucket_start, '+00:00', ?)) AS bucket_date,
		...
		LEFT JOIN (
			...
			GROUP BY bucket_date
		) AS user_counts ON user_counts.bucket_date = daily.bucket_date
		ON DUPLICATE KEY UPDATE
			total_requests = VALUES(total_requests), input_tokens = VALUES(input_tokens),
			output_tokens = VALUES(output_tokens), cache_creation_tokens = VALUES(cache_creation_tokens),
			cache_read_tokens = VALUES(cache_read_tokens), total_cost = VALUES(total_cost),
			...
			computed_at = VALUES(computed_at)
	`
	_, err := r.sql.ExecContext(ctx, query, tzName, start, end, start, end)
	return err
}
...
func (r *dashboardAggregationRepository) dropUsageLogsPartitions(ctx context.Context, cutoff time.Time) error {
	...
	for rows.Next() {
		...
		if month.Before(cutoffMonth) {
			if _, err := r.sql.ExecContext(ctx, fmt.Sprintf("ALTER TABLE usage_logs DROP PARTITION %s", quoteMySQLIdentifier(name))); err != nil {
				return err
			}
		}
	}
...
func (r *dashboardAggregationRepository) createUsageLogsPartition(ctx context.Context, month time.Time) error {
	...
	nextMonth := monthStart.AddDate(0, 1, 0)
	name := fmt.Sprintf("p%s", monthStart.Format("200601"))
	query := fmt.Sprintf(
		"ALTER TABLE usage_logs ADD PARTITION (PARTITION %s VALUES LESS THAN (TO_DAYS('%s')))",
